...
/* 				alert("만난다"); */
		...
		$('[data-toggle=offcanvas]').click(
				function() {
					...
					$('#lg-menu').toggleClass('hidden-xs').toggleClass(
							'visible-xs');
					$('#xs-menu').toggleClass('visible-xs').toggleClass(
							'hidden-xs');
					$('#btnShow').toggle();
				});
 ...
 */		getBoardScroll(story);
						...
						$
								.each(
										...
										function(key, value) {
											...
											a += '<div class="commentInfo'+value.reply_seq+'">'
													+ '댓글번호 : '
													+ value.reply_seq
													+ ' / 작성자 : '
													+ value.rwriter;
											a += '<button onclick="replyUpdate('
													+ value.reply_seq
													+ ',\''
													+ value.rcontent
													+ '\','
											...
											a += '<button onclick="replyDelete('
													...
													+ value.story_seq
													+ ');"> 삭제 </button> </div>';
											a += '<div class="commentContent'+value.reply_seq+'"> <p> 내용 : '
													+ value.rcontent + '</p>';
											a += '</div></div>';
											$(
													'[name=commentList'
	...
	function replyDelete(reply_seq, board_seq, story_seq) {
		$.ajax({
			...
			success : function(data) {
				if (data == 1)
					replyList(board_seq, story_seq); //댓글 삭제후 목록 출력 
			}
		});
	}
...
<body>
		...
		<div class="box">
			<div class="row row-offcanvas row-offcanvas-left">
				...
				<div id="main" style="overflow-y: auto;"
					class="column col-sm-12 col-xs-12">
					<div class="column col-sm-2 col-xs-1 sidebar-offcanvas"
						id="sidebar" name="story"
						style="position: fixed; margin-top: 74px;">
						<!-- <ul class="accodian">
							<ul class="accodian">
								<input value="39" name="storyHidden" type="hidden">
								<h3 onclick="story_button(39)">
									<a href="#">#asdasdas</a>
								</h3>
							</ul>
						</ul>
						<ul class="accodian">
							<ul class="accodian">
								<input value="7" name="storyHidden" type="hidden">
								<h3 onclick="story_button(7)">
									<a href="#">#ㅁㄴㅇㄹ</a>
								</h3>
							</ul>
						</ul>
						<ul class="accodian">
							<ul class="accodian">
								<input value="1" name="storyHidden" type="hidden">
								<h3 onclick="story_button(1)">
									<a href="#">#강남역 맛집</a>
								</h3>
							</ul>
						</ul>
						<ul class="accodian">
							<ul class="accodian">
								<input value="5" name="storyHidden" type="hidden">
								<h3 onclick="story_button(5)">
									<a href="#">#고고고</a>
								</h3>
							</ul>
						</ul>
						<ul class="accodian">
							<ul class="accodian">
								<input value="10" name="storyHidden" type="hidden">
								<h3 onclick="story_button(10)">
									<a href="#">#동대문</a>
								</h3>
							</ul>
						</ul>
						<ul class="accodian">
							<ul class="accodian">
								<input value="37" name="storyHidden" type="hidden">
								<h3 onclick="story_button(37)">
									<a href="#">#민웅스 스퇼</a>
								</h3>
							</ul>
						</ul>
						<ul class="accodian">
							<ul class="accodian">
								<input value="6" name="storyHidden" type="hidden">
								<h3 onclick="story_button(6)">
									<a href="#">#오오오오오</a>
								</h3>
							</ul>
						</ul>
						<ul class="accodian">
							<ul class="accodian">
								<input value="2" name="storyHidden" type="hidden">
								<h3 onclick="story_button(2)">
									<a href="#">#제주도 여행</a>
								</h3>
							</ul>
						</ul>
						<ul class="accodian">
							<ul class="accodian">
								<input value="36" name="storyHidden" type="hidden">
								<h3 onclick="story_button(36)">
									<a href="#">#지니지니</a>
								</h3>
							</ul>
						</ul> -->
	 ...
	 */		if (keyword == "") {
		...
		var accModule = function() {
			...
			var acc_wrap = $('.accodian'), question = acc_wrap.find('h3'), answer = question
					.next('div');
